...
public class Proportional { // Przydział proporcjonalny
    DefProcess[] frames;
    ...
    public Proportional(DefProcess[] frames, ArrayList<DefProcess> defProcessList) {
        this.frames = frames;
    ...
    private void start(){
        ...
        for (int i = 0; i < defProcessList.size(); i++) {
            ...
            int add = min + assingFrames(frames.length, countAllReferences(), defProcessList.get(i).getReferenceList().size());
    ...
    // liczenie stosunku ilosci ramek do ilosci procesow
    private int assingFrames(int countFrames, int allReferences, int thisReferences){
        int resoult = (countFrames*thisReferences)/allReferences;

        return resoult;
    }

    // ilosc wszystkich odwolan
    private int countAllReferences(){
        int resoult = 0;

        for (int i = 0; i < defProcessList.size(); i++)
            resoult += defProcessList.get(i).getReferenceList().size();
    ...
    public String getMissingPage() {
        return "--> Brakujacych stron w przydziale proporcjonalnym: "+missingPage;
    }
